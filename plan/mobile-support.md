...
The web PWA build stays intact. Capacitor wraps the _same_ web build in a native
...
- **Minimum OS:** Android 7.0 (API 24) and iOS 15 (Capacitor 6 defaults). Confirm against device targets.
- **Capacitor major version:** Version 8.x.
...
- **Web PWA stays published:** yes. Capacitor is an _additional_ target, not a replacement.
...
**Goal:** Adjust the existing web app so the _same_ build works both as the
...
1. Native shell → 4. File import + open-with → 5. Touch/responsive UX →
2. Icons/splash → 7. Storage durability → 8. Build/signing → 9. QA matrix.
